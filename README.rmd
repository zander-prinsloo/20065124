...
The data files used here are not uploaded to GitHub. However, the data is loaded for use in the README below:

```{r}
# Location of data
Loc <- "/Users/zanderprinsloo/Library/Mobile Documents/com~apple~CloudDocs/Desktop/Desktop – MacBookPro’s MacBook Pro/Academic/Postgraduate/Masters/Modules/Data Science/mock-test/mock-20065124/data/Movies/"
...
# 
# 
# df_movies <- fn_Data_Collate(Datroot = "Question1/Data/!!NAME!!/") %>% 
#   as.list() %>% 
#   walk(~source(.))
```

In Question 1 I will be analysing the Movies data set. It has 72 observations which corresponds to 72 different films. There are 8 variables, which are `r colnames(df_movies)`. Therefore, we can analyse the success of each film by criteria such as ratings by audiences, ratings by Rotten Tomatoes, Profitability, etc. 

### Question 1a

Plot here:

```{r Figure1,  warning =  FALSE, fig.align = 'center', fig.cap = "Happy Plot \\label{Figure1}", fig.ext = 'png', fig.height = 7, fig.width = 8}


g <- Plot_Happy_ErrorBar(df_happy, 
                    title = "Ladder Score by Region", 
                    subtitle = "Happiness Results", 
